...
// Corrige erro de sessão caindo no Render
...
app.use(session({
    secret: 'segredo_sonda_saas_v2_final',
    resave: false,
...
// --- 2. AUTO-REPARO DO BANCO (CRUCIAL) ---
async function iniciarSistema() {
    try {
        console.log('>>> 🚀 INICIANDO SONDASAAS (MONOLITO)...');
        ...
        // Garante Colunas V2 (Corrige erro "column does not exist")
        const updates = [
        ...
        console.log('>>> ✅ SISTEMA PRONTO E BANCO ATUALIZADO.');
    } catch (e) { console.error('!!! ERRO NO STARTUP !!!', e); }
}

// --- 3. GERADOR DE PDF (LAYOUT SONDAMAIS) ---
const montarLayoutPDF = (doc, p, empresa) => {
    ...
    y += 12;
    // ID curto para o número da proposta
    ...
    doc.text(p.furos_previstos ? p.furos_previstos.toString() : '0', col.QTD, y);
    ...
    doc.font('Helvetica');
    doc.text(p.metragem_total ? p.metragem_total.toString() : '0', col.QTD, y);
    ...
    // Extras
    y += 20; doc.text('ART', col.DESC, y); doc.text('1', col.QTD, y); doc.text(`R$ ${art.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`, col.UNIT, y); doc.text(`R$ ${art.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`, col.TOTAL, y);
    y += 20; doc.text('Mobilização', col.DESC, y); doc.text('1', col.QTD, y); doc.text(`R$ ${mob.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`, col.UNIT, y); doc.text(`R$ ${mob.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`, col.TOTAL, y);
...
// Propostas (Listar, Criar, PDF, Status, Deletar)
...
// Fallback PDF antigo
app.get('/gerar-pdf/:id', checkAuth, (req, res) => res.redirect(`/api/propostas/${req.params.id}/pdf`));
...
// --- START ---
iniciarSistema().then(() => {
    app.listen(port, () => { console.log(`>>> 🚀 SONDASAAS MONOLITO V2 RODANDO NA PORTA ${port}`); });
});