...
app.use(express.urlencoded({ extended: true })); // Para ler form POST
...
app.post('/gerar-proposta', async (req, res) => {
    ...
    try {
        ...
        const dadosPDF = {
            ...
            ...d, // Pega todos os campos do form
...
app.get('/reemitir-pdf/:id', async (req, res) => {
    ...
    try {
        ...
        const dadosPDF = {
            ...
            // Recupera critério do banco (ou assume norma se for antigo)
...
function gerarPDFDinamico(res, d) {
    ...
    // LÓGICA DE TEXTO DA TABELA (Baseado no Critério)
    ...
    // TEXTOS JURÍDICOS (DINÂMICOS AGORA!)
    doc.moveDown(2); checkPageBreak(100); 

    // Aqui está a mágica: Texto muda conforme a escolha
    if (d.criterio === 'cota') {
...
// --- INIT (AUTO MIGRATION PARA COLUNAS NOVAS) ---
const initSQL = `
    CREATE TABLE IF NOT EXISTS propostas (
        id SERIAL PRIMARY KEY, cliente VARCHAR(255), endereco TEXT, furos INTEGER, metragem_total NUMERIC, 
        valor_art NUMERIC, valor_mobilizacao NUMERIC, valor_desconto NUMERIC, valor_total NUMERIC, 
    ...
    /* NOVAS COLUNAS - Executarão automaticamente */
    ALTER TABLE propostas ADD COLUMN IF NOT EXISTS criterio VARCHAR(50);
    ALTER TABLE propostas ADD COLUMN IF NOT EXISTS detalhe_criterio VARCHAR(255);
`;

pool.query(initSQL).then(() => { console.log('>>> DB E MIGRATION OK <<<'); app.listen(port, () => { console.log(`Rodando na porta ${port}`); }); }).catch(err => { console.error('ERRO DB:', err); });